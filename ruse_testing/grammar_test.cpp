...
form or some other expression that produces a transformer.
  ...
  TEST(grammar, scheme)
  {
    ...
    constexpr auto scheme = grammar{
      ...
      comment<
        "*Definitions*.  Definitions include variable and syntax definitions, "
        "begin forms containing",
        "zero or more definitions, let-syntax and letrec-syntax forms "
        "expanding into zero or more",
        "definitions, and derived definitions. Derived definitions are "
        "syntactic extensions that",
        "expand into some form of definition. A transformer expression is a "
        "syntax-rules form or",
      ...
      rule<"syntax definition">(
        parens(definesyntax, "keyword"_n, "transformer expression"_n)),
      rule<"keyword">("identifier"_n),
      rule<"syntax binding">(parens("keyword"_n, "transformer expression"_n)),

      comment<
        "*Expressions*.  Expressions include core expressions, let-syntax or "
        "letrec-syntax forms expanding",
        "into a sequence of one or more expressions, and derived expressions. "
        "The core expressions are",
        "self-evaluating constants, variable references, applications, and "
        "quote, lambda, if, and set! expressions.",
        "Derived expressions include and, begin, case, cond, delay, do, let, "
        "let*, letrec, or, and quasiquote",
        "expressions plus syntactic extensions that expand into some form of "
        "expression.">,
      ...
      comment<
        "*Identifiers*.  Identifiers may denote variables, keywords, or "
        "symbols, depending upon context.",
        "They are formed from sequences of letters, digits, and special "
        "characters. With three exceptions",
        "identifiers cannot begin with a character that can also begin a "
        "number, i.e., they cannot begin",
        "with ., +, -, or a digit. The three exceptions are the identifiers "
        "..., +, and -. Case is insignificant",
        "in symbols so that, for example, newspaper, NewsPaper, and NEWSPAPER "
        "all represent the same identifier.">,

      comment<
        "*Data*.  Data include booleans, numbers, characters, strings, "
        "symbols, lists, and vectors. Case is",
        "insignificant in the syntax for booleans, numbers, and character "
        "names, but it is significant in",
        "other character constants and in strings. For example, #T is "
        "equivalent to #t, #E1E3 is equivalent",
        "to #e1e3, #X2aBc is equivalent to #x2abc, and #\\NewLine is "
        "equivalent to #\\newline; but #\\A is distinct",
        "from #\\a and \"String\" is distinct from \"string\".">,

      comment<
        "Numbers.  Numbers can appear in one of four radixes: 2, 8, 10, and "
        "16, with 10 the default. The first",
        "several of productions below are parameterized by the radix, r, and "
        "each represents four productions,",
        "one for each of the four possible radixes. Numbers that contain radix "
        "points or exponents are constrained",
        "to appear in radix 10, so <decimal r> is valid only when r is 10.">,

      rule<"num(r)">(
        "real(r)"_n,
        ("real(r)"_n, "@"_t, "read(r)"_n),
      ...
      rule<"real(r)">(("sign"_n, "ureal(r)"_n))
        ...
        ,
      rule<"ureal(r)">(
        "integer(r)"_n, ("integer(r)"_n, "/"_t, "integer(r)"_n), "decimal(r)"_n)
      ...
      rule<"uinteger(r)">((+"digit(r)"_n, *"#"_t))
        ...
        ,
      rule<"prefix(r::rad)">(
        ("radix(r::rad)"_n, "exactness"_n), ("exactness"_n, "radix(r::rad)"_n))
