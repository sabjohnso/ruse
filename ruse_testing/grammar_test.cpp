...
form or some other expression that produces a transformer.
  ...
  TEST(zero_or_more, nonterminal){
    STATIC_EXPECT_EQ(
      zero_or_more<nonterminal<"form">>{}, *n<"form">);
  }
  ...
  TEST(one_or_more, nonterminal){
    STATIC_EXPECT_EQ(
      one_or_more<nonterminal<"form">>{}, +n<"form">);
  }
  ...
  TEST(zero_or_one, nonterminal){
    STATIC_EXPECT_EQ(
      zero_or_one<nonterminal<"form">>{}, ~n<"form">);
  }
  ...
  TEST(production_rule, construction){
    STATIC_EXPECT_EQ(
      (production_rule<
       ...
       sequence<terminal<"(">, terminal<"quote">, nonterminal<"datum">, terminal<")">>>{}),
      n<"expression"> <=> n<"constant"> | n<"variable"> | (t<"(">, t<"quote">, n<"datum">, t<")">));
  }
  ...
  TEST(grammar, construction){
    ...
    EXPECT_EQ(
      (grammar<
       production_rule<nonterminal<"program">, zero_or_more<nonterminal<"form">>>,
       production_rule<nonterminal<"form">, nonterminal<"definition">, nonterminal<"expression">>>{}),
      (grammar{
        n<"program"> <=> *n<"form">,
  ...
  TEST(grammar, scheme){
    ...
    constexpr auto scheme =
      ...
      ,rule<"ureal(r)">(
        "integer(r)"_n,
        ("integer(r)"_n, "/"_t, "integer(r)"_n),
        "decimal(r)"_n)

      ,rule<"uinteger(r)">((+"digit(r)"_n, *"#"_t))

      ,rule<"prefix(r::rad)">(
        ("radix(r::rad)"_n, "exactness"_n),
        ("exactness"_n, "radix(r::rad)"_n))
