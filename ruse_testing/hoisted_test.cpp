...
namespace ruse::testing {
   ...
   TEST(hoisted_list, lower) {
      STATIC_EXPECT_EQ("abc"_s, lower("abc"_hoist));
   }

   TEST(hoisted_list, hoist) {
      STATIC_EXPECT_TRUE(
        is_hoisted_list(hoist([] { return "abc"_s; })));
   }
   ...
   TEST(hoisted_list, hoisted_append) {
      STATIC_EXPECT_EQ(
        "abcdef"_hoist,
        hoisted_append("abc"_hoist, "def"_hoist));
   }

   TEST(hoisted, hoisted_head) {
      STATIC_EXPECT_EQ(
        hoist([] { return 'a'; }),
        hoisted_head("abc"_hoist));
   }
   ...
   TEST(hoisted, hoisted_take) {
      STATIC_EXPECT_EQ(
        "abc"_hoist,
        hoisted_take(nat<3>, "abc123"_hoist));
   }

   TEST(hoisted, hoisted_drop) {
      STATIC_EXPECT_EQ(
        "123"_hoist,
        hoisted_drop(nat<3>, "abc123"_hoist));
   }

   TEST(hoisted_List, fmap) {
      EXPECT_EQ(
        "AB"_hoist,
        fmap([](auto x) -> char { return x - 32; }, "ab"_hoist));
   }
