...
namespace ruse::testing {
  ...
  TEST(pure, fapply)
  {
    EXPECT_EQ(pure(9), fapply(pure([](auto x) { return x * x; }), pure(3)));
  }
  ...
  TEST(pure, duplicate) { STATIC_EXPECT_EQ(pure(pure(3)), duplicate(pure(3))); }

  TEST(pure, extend) { STATIC_EXPECT_EQ(pure(3), extend(extract, pure(3))); }
