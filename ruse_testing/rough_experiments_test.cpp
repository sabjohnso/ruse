...
namespace ruse::experimental::testing {
  ...
  struct MyNode
  {
    ...
    // `fn1` is a function that accepts two non-literal values and mutates one
    // of them.  However, the function itself does not capture any values
    // and is constexpr. Is has no impact on the literal nature of `MyNode`
    static constexpr auto fn1 = [](const std::string& input, State& state) {
      state.value = input;
    };
  ...
  TEST(callable_struct, to_type)
  {
    ...
    STATIC_EXPECT_EQ("a"_s, list_ref(nat<0>, T::value()));
    std::cout << type_name<decltype(list_ref(nat<0>, T::value()))> << std::endl;
    std::cout << type_name<decltype(list_ref(nat<1>, T::value()))> << std::endl;
    ...
    std::cout << get_signature([](int x, int y) { return x + y; }) << std::endl;
    std::cout << get_signature([] {}) << std::endl;
    std::cout << get_signature([](int x) { return x; }) << std::endl;

    // We still can't get out the generic arguments but we could try something
    // like what is done in reflection to get the fields of a struct

    // We could try to have a type called anything and otherone that can't be
    // cast to anything and see what the minimum number of arguments is and what
    // the defined types are. It seems problematic
  ...
  // The folling could be technically feasible, but it is not very readablea
  // list(
  //   define_syntax<"begin/m">(
  //     syntax_parser(
  //       pattern((_, "e"_expr)).repl(stx<"e"_expr>)
  //       pattern((_, "e"_expr, +"es"_expr)).repl(
  //         app("with_syntax"_s, list(list("ignored"_s, generate_temporary())),
  ...
  //       rule(patt(ap(_, l(l("x"_id, "mx"_expr), +"more-bidings"_expr),
  //       "body"_expr)),
  ...
  template<typename T>
  concept Unembelished =
    std::is_same_v<T, std::remove_cvref_t<T>> and not std::is_pointer_v<T> and
  ...
  //
  // We would also want to know the arrays, because we will need to construct
  // Them from their element type.  Boost DI would produce an array of
  // equal values, because it only makes one of each type.
  ...
  //   dolist(
  //     fs, [&]<auto F>(auto&& f) {
  //       if constexpr (Invocable<decltype(f), decltype(x), decltype(gss)...>)
  //       {
  //         f(x, gss...);
  //       }
  //     });
  ...
  TEST(emit, xxx)
  {
    ...
    auto increment = [](int input, auto&&... ks) { emit(input + 1, ks...); };
    auto sqr = [](int input, auto&&... ks) { emit(input * input, ks...); };
