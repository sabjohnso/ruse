...
namespace ruse::testing {
   ...
   TEST(tagged, construction) {
      STATIC_EXPECT_TRUE(is_tagged("abc"_tag(4)));
   }

   TEST(tagged, fmap) {
      STATIC_EXPECT_EQ(
        "abc"_tag(4),
        fmap([](auto x) { return x + 1; }, "abc"_tag(3)));
   }
   ...
   TEST(tagged, letm) {
      STATIC_EXPECT_EQ("c"_tag(4), letm("a"_tag(1), [](auto x) {
                          return letm("b"_tag(3), [ = ](auto y) {
                             return "c"_tag(x + y);
                          });
                       }));
   }
