...
namespace ruse::testing {
   ...
   TEST(state, make_state) {
      STATIC_EXPECT_EQ(
        list('x', 's'),
        run_state('s')(make_stateful('x')));
   }

   TEST(state, fmap) {
      STATIC_EXPECT_EQ(
        list(2, 's'),
        run_state('s', fmap(twc, make_stateful(1))));
   }
   ...
   TEST(state, put) {
      STATIC_EXPECT_EQ(
        list(nothing, 's'),
        run_state('t')(put_state('s')));
   }

   TEST(state, modify) {
      STATIC_EXPECT_EQ(
        list(nothing, 2),
        run_state(1)(modify_state(twc)));
   }
   ...
   TEST(state, game) {
      ...
      static constexpr auto update_state =
        curry(nat<2>, [](auto in, auto state) {
           return is_on(state)
                  ? (in == input::off
                       ...
                       : (in == input::up
                            ...
                            : (in == input::down
                                 ? list(status::on, score(state) - 1)
                                 : state)))
                  : (in == input::on ? list(status::on, score(state))
                                     : state);
        });
      ...
      STATIC_EXPECT_EQ(
        list(status::on, 0),
        update_state(input::on, list(status::off, 0)));

      STATIC_EXPECT_EQ(
        list(status::on, 1),
        update_state(input::up, list(status::on, 0)));

      STATIC_EXPECT_EQ(
        list(status::on, -1),
        update_state(input::down, list(status::on, 0)));
      ...
      static constexpr auto game = [](List auto inputs) {
         constexpr auto recur = []<List T>(auto recur, T inputs) {
            if constexpr(Nothing<T>) {
               ...
               return beginm(
                 modify_state(update_state(car(inputs))),
                 recur(recur, cdr(inputs)));
            }
         };
