...
namespace ruse::experimental {
  ...
  requires(
    Lower < Upper &&
    is_same_v<decltype(Lower), decltype(Upper)>) struct character_range
  {
  ...
  constexpr auto left_adjacent = curry(
    nat<2>,
    []<CharacterClassPrimitive C1, CharacterClassPrimitive C2>(C1, C2) {
      if constexpr (CharacterValue<C1> && CharacterValue<C2>) {
  ...
  constexpr auto right_adjacent = curry(
    nat<2>,
    []<CharacterClassPrimitive C1, CharacterClassPrimitive C2>(C1 x, C2 y) {
