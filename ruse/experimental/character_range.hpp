...
namespace ruse::experimental{
  ...
  struct character_value
  {
    ...
    contains(character_value){ return true; }

    template<character_type OtherValue>
    static constexpr bool
    contains(character_value<OtherValue>){ return false; }

    static constexpr bool
    contains(character_type input){ return input == value; }
  };
  ...
  struct {
    template<Character auto Value>
    constexpr bool
    operator()(type_s<character_value<Value>>) const { return true; }
  ...
  constexpr auto is_character_value = []<typename T>(T){ return CharacterValue<T>; };
  ...
  requires(Lower < Upper && is_same_v<decltype(Lower), decltype(Upper)>)
  ...
  struct {
    ...
    template<Character auto Lower, Character auto Upper>
    constexpr bool
    operator()(type_s<character_range<Lower, Upper>>) const { return true; }
  ...
  constexpr auto is_character_range = []<typename T>(T){ return CharacterRange<T>; };

  template<Character auto Lower, Character auto Upper>
  requires(Lower < Upper && is_same_v<decltype(Lower), decltype(Upper)>)
  constexpr character_range<Lower, Upper> range{};

  template<typename T>
  concept CharacterClassPrimitive = CharacterValue<T> || CharacterRange<T>;
  ...
  operator<(C1, C2){
    if constexpr (CharacterValue<C1> && CharacterValue<C2>){
      ...
      return (C1::lower < C2::lower) || ((C1::lower == C2::lower) && (C1::upper < C2::upper));
  ...
  constexpr auto left_adjacent = curry(nat<2>, []<CharacterClassPrimitive C1, CharacterClassPrimitive C2>(C1, C2){
      if constexpr (CharacterValue<C1> && CharacterValue<C2>){
  ...
  constexpr auto right_adjacent = curry(nat<2>, []<CharacterClassPrimitive C1, CharacterClassPrimitive C2>(C1 x, C2 y){
      return left_adjacent(y, x);
    });

  constexpr auto adjacent = curry(nat<2>, []<CharacterClassPrimitive C1, CharacterClassPrimitive C2>(C1 x, C2 y){
      return left_adjacent(x, y) || right_adjacent(x, y);
    });

  constexpr auto overlap = curry(nat<2>, []<CharacterClassPrimitive C1, CharacterClassPrimitive C2>(C1, C2){
      if constexpr (CharacterValue<C1> && CharacterValue<C2>){
