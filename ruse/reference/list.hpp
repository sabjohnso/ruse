...
namespace ruse::reference {
   ...
   /**
    * @brief Return `true` if the input is a list.  Otherwise, return
    * `false`.
    */
   ...
   /**
    * @brief Return `true` if the input is a nonempty list. Otherwise,
    * return `false`.
    */
   ...
   /**
    * @brief Return `true` if the input list is an empty list.
    * Otherwise, return `false`.
    */
   constexpr auto is_empty_list = []<typename T>(T) {
      return EmptyList<T>;
   };

   /**
    * @brief Return `true` if the input type proxy is a proxy for an
    * empty list type. Otherwise, return `false`.
    */
   ...
   constexpr auto length_type = []<List T>(Type<T>) {
      constexpr auto aux =
        []<typename U>(auto recur, auto accum, Type<U>) {
           if constexpr(Pair<U>) {
              return recur(
                recur,
                accum + 1,
                type<typename U::second_type>);
           } else {
              return accum;
           }
        };
   ...
   /**
    * @brief Return `true` if the input is a unitary list.  Otherwise,
    * return `false`.
    */
   ...
   /**
    * @brief Return `true` if every member of the input list type proxy
    * is a vacuous type. Otherwise, return `false`.
    */
   ...
   /**
    * @brief The concept of a vacuous list: a list where every member is
    * an instance of a vacuous type.
    */
   template<typename T>
   concept VacuousList = List<T> and is_vacuous_list_type(type<T>);

   /**
    * @brief Return `true` if every member of the input list is a
    * vacuous type.
    */
   constexpr auto is_vacuous_list = []<typename T>(T) {
      return VacuousList<T>;
   };

   /**
    * @brief Return `true` if the input type proxy is for a homogeneous
   ...
   /**
    * @brief Return `true` if the input is a homogeneous list.
    * Otherwise, return `false`.
    */
   ...
   /**
    * @brief Return `true` if the input is an association list type.
    * Otherwise, return `false`.
    */
    ...
    */
   template<typename T>
   concept AssociationList = List<T> &&
     is_association_list_type(type<T>);

   /**
    * @brief Return `true` if the input is an association list.
    * Otherwise, return `false`.
    */
    ...
    */
   template<typename T>
   concept HomogeneousAssociationList =
     HomogeneousList<T> && AssociationList<T>;

   /**
    * @brief Return `true` if the input is a homogeneous association
    * list. Otherwise, return `false`.
    */
   ...
   /**
    * @brief Return `true` if the input is a type proxy for a property
    * list type. Otherwise, return `false`.
    */
   ...
   /**
    * @brief Return `true` if the input is a property list. Otherwise,
    * return `false`.
    */
   ...
   /**
    * @brief Return a list containing all but the first element of the
    * input list.
    */
   ...
   /**
    * @brief Return the reverse of the first input list appended with
   ...
   constexpr auto rappend = curry(
     ...
     [](List auto xs, List auto ys) -> List auto{
        constexpr auto aux =
          []<List T>(auto recur, T xs, List auto ys) {
             if constexpr(Pair<T>) {
   ...
   constexpr auto append =
     curry(nat<2>, [](List auto xs, List auto ys) {
   ...
   constexpr auto list = [](auto... xs) -> List auto{
      ...
      constexpr auto aux =
        [](auto recur, List auto accum, auto x, auto... xs) {
   ...
   /**
    * @brief Return a vacuous list with the elements sorted according to
    * cmp.
    */
   ...
   /**
    * @brief Return the results of filtering the input vacuous list with
    * the input predicate.
    */
   ...
   /**
    * @brief Return the with elements taken from the input list while
    * the input predicate is true.
    */
   ...
   /**
    * @brief Return a list containing elements from the start of the
    * input list until the first element is reach that satisfies the
    * input predicate.
    */
   constexpr auto vacuous_list_take_until =
     curry(nat<2>, []<VacuousList T>(auto pred, T) {
        return vacuous_list_take_while(
          [ = ](auto x) { return not pred(x); },
          T{});
     });
    ...
    */
   constexpr auto list_ref =
     curry(nat<2>, []<integer N, List T>(Nat<N>, T xs) {
        static_assert(
          N < length_type(type<T>),
          "Expected a valid index for the input list");

        constexpr auto aux =
          []<integer M>(auto recur, Nat<M>, List auto xs) {
             if constexpr(M == 0) {
                return head(xs);
             } else {
   ...
   /**
    * @brief Return a list containing the first `n` elements of the
    * input list, or return the input list if it has `n` or fewer
    * elements
    */
   ...
   constexpr auto drop = curry(
     ...
     []<integer N>(Nat<N>, List auto xs) -> List auto{
        constexpr auto aux =
          []<integer M, List T>(auto recur, Nat<M>, T xs) {
             if constexpr(M == 0 || Nothing<T>) {
                return xs;
             } else {
                return recur(recur, nat<M - 1>, tail(xs));
   ...
   /**
    * @brief Return the indicated element from the homogeneous input
    * list.
    */
   constexpr auto homogeneous_list_ref =
     curry(nat<2>, [](integer index, HomogeneousList auto xs) {
        constexpr auto recur =
          []<NonemptyList T>(auto recur, integer index, T xs) {
             if constexpr(UnitaryList<T>) {
                return head(xs);
             } else {
                return index > 0 ? recur(recur, index - 1, tail(xs))
                                 : head(xs);
             }
          };
   ...
   constexpr auto homogeneous_list_sort =
     curry(nat<2>, []<HomogeneousList T>(auto cmp, T xs) {
        constexpr auto recur = []<List Left, List Right>(
                                 auto recur,
                                 auto cmp,
                                 Left left,
                                 Right right) -> T {
   ...
   constexpr auto property_list_has_tag =
     curry(nat<2>, [](Tag auto key, PropertyList auto xs) {
        constexpr auto recur =
          []<Tag K, PropertyList T>(auto recur, K key, T xs) {
   ...
   /**
    * @brief Return the first element of the input property list
   ...
   /**
    * @brief Return the value from the first element of the input
    * property list matching the input tag.
    */
    ...
    */
   constexpr auto last = curry(nat<1>, [](NonemptyList auto xs) {
      return head(reverse(xs));
   });

   /**
    * @brief Return a list with all but the last element of the input
    * list.
    */
   ...
   /**
    * @brief Exectute the input function over the input list purely for
    * side effects.
    */
   ...
   /**
    * @brief Return the concatenation of multiple copies of the input
    * list
    */
   template<integer N>
   constexpr auto
   operator*(Nat<N>, List auto xs) {
      constexpr auto recur = []<integer I>(
                               auto recur,
                               Nat<I>,
                               List auto xs,
                               List auto accum) {
   ...
   /**
    * @brief Return the left fold of the input list with the input
    * function and initial value.
    */
   constexpr auto foldl =
     curry(nat<3>, [](auto f, auto init, List auto xs) {
        constexpr auto recur =
          []<List T>(auto recur, auto f, auto init, T xs) {
   ...
   get_flatmap(Type<T>) {
      return [](auto f, List auto xs) -> List auto{
         constexpr auto aux =
           []<List U>(auto recur, List auto accum, auto f, U xs) {
              if constexpr(NonemptyList<U>) {
                 return recur(
                   recur,
                   rappend(f(head(xs)), accum),
                   f,
                   tail(xs));
              } else {
                 return reverse(accum);
   ...
   get_zapply(Type<T>) {
      return []<NonemptyList F>(F wf, ListLike<F> auto wx) {
         return [=]<auto... Indices>(index_sequence<Indices...>) {
            return list(list_ref(nat<Indices>, wf)(
              list_ref(nat<Indices>, wx))...);
         }
