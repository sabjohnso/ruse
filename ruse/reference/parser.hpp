...
namespace ruse::reference {
   ...
   constexpr auto parse_result_value =
     [](PropertyList auto parse_result) {
        return property_list_ref("value"_tag, parse_result).value;
     };

   constexpr auto parse_result_sequence =
     [](PropertyList auto parse_result) {
   ...
   struct result_parser {
      ...
      operator()(Sequence) const {
         return list(
           list("value"_tag(T{}), "sequence"_tag(Sequence{})));
      }
   };
   ...
   constexpr auto conj = curry(
     nat<2>,
     []<Vacuous Parser, Vacuous ParserConstructor>(
       Parser,
       ParserConstructor) {
   ...
   constexpr auto disj = curry(
     nat<2>,
     []<Vacuous Parser1, Vacuous Parser2>(Parser1, Parser2) {
   ...
   constexpr auto opt = curry(nat<1>, [](Vacuous auto p) {
      return alt(p, result(nothing));
   });
