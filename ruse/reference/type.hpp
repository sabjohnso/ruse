...
namespace ruse::reference {
   ...
   template<typename T>
   concept NonCVRef =
     Nonreference<T> and Nonvolatile<T> and Nonconst<T>;
   ...
   /**
    * @brief Return `true` if the input is a type proxy. Otherwise,
    * return `false`.
    */
   ...
   concept TypeProxyOf =
     is_same_v<remove_cvref_t<T>, Type<remove_cvref_t<U>>>;
   ...
   constexpr auto
   get_flatmap(Type<T>) {
      return []<typename F, typename U>(F, Type<U>) {
