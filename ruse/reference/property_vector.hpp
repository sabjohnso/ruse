...
namespace ruse::reference {
  ...
  /**
   * @brief Return true if the input is a type proxy for a type that is a
   * property vector.
   */
      ...
      return u(
        []<class U>(auto recur, Type<U>) {
          if constexpr (NonemptyVector<U>) {
            if constexpr (is_tagged_type(flatmap(back_type, type<U>))) {
              return recur(recur, fmap(vector_head_type, type<U>));
            } else {
              return false;
            }
          } else {
  ...
  /**
   * @brief Return `true` if the input is a property vector. Otherwise, return
   * `false`.
   */
  ...
  /**
   * @brief Return `true` if the property vector has a value with the indicated
   * tag. Otherwise, return `false`.
   */
  ...
  /**
   * @brief Return the tagged value from the property vector if an element with
   * the tag is present.  Otherwise, return `false`.
   */
