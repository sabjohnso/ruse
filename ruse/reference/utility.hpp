...
namespace ruse::reference
{
...
#else
          constexpr auto apply_taking = []<std::size_t ... Indices>(index_sequence<Indices ...>){
            return [](auto g, auto zs){
              return g(std::get<Indices>(zs) ...);
            };
          }(make_index_sequence<N>());

          constexpr auto apply_dropping = []<std::size_t ... Indices>(index_sequence<Indices ...>){
            return [](auto g, auto zs){
   ...
   */
  constexpr auto compose = curry(nat<2>, [](auto f, auto g){ return
        [=](auto ... xs){ return f(g(xs ...)); }; });

  /**
   * @brief Return the result of applying the input function to the input result.
   */
  constexpr auto let = [](auto x, auto f){
    return f(x);
  };
