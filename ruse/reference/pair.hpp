...
namespace ruse::reference {
  ...
  concept Pair = requires(T x)
  {
    ...
    {
      x.first
      } -> convertible_to<typename T::first_type>;
    {
      x.second
      } -> convertible_to<typename T::second_type>;
  };
  ...
  /**
    @brief Return `true` if the input is a pair. Otherwise, return `false`.
   */
   ...
   */
  constexpr auto cons = curry(nat<2>, []<typename T, typename U>(T x, U y) {
   ...
   */
  constexpr auto car = curry(nat<1>, [](Pair auto xs) { return xs.first; });

  /**
   * @brief Return the first member from a pair.
   */
  constexpr auto cdr = curry(nat<1>, [](Pair auto xs) { return xs.second; });
