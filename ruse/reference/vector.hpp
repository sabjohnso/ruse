...
namespace ruse::reference {
   ...
   /**
    * @brief Return `true` if the input is a type proxy for a
    * specialization of a class template.  Otherwise, return `false`.
    */
   ...
   /**
    * @brief Return `true` if the input is a type proxy for an empty
    * vector type. Otherwise,  return `false`.
    */
   constexpr auto is_empty_vector_type = []<typename T>(T) {
      if constexpr(TypeProxy<T>) {
         return
           []<typename U>(Type<U>) { return U{} == empty_vector; }(T{});
      } else {
         return false;
      }
   };
   ...
   /**
    * @brief Return `true` if the input is an empty vector. Otherwise,
    * return `false`.
    */
   constexpr auto is_empty_vector = []<typename T>(T) {
   ...
   /**
    * @brief Return `true` if the input is a type proxy for a vector
    * head. Otherwise, return `false`.
    */
   ...
   /**
    * @brief Return `true` if the input is a unitary vector. Otherwise,
    * return `false`.
    */
   ...
   /**
    * @brief Return true if the input is a type proxy for a nonempty
    * vector
    */
    ...
    */
   template<typename T>
   concept NonemptyVector = UnitaryVector<T> ||
     is_nonempty_vector_type(type<T>);
   ...
   constexpr auto vector_type_length = []<Vector T>(Type<T>) {
      return u(
        case_lambda(
          []<EmptyVector U, integer N>(auto, Type<U>, Nat<N>) {
             return N;
          },
          []<UnitaryVector U, integer N>(auto, Type<U>, Nat<N>) {
             return N + 1;
          },
          []<Vector Head, typename Tail, integer N>(
            auto recur,
            Type<pair<Head, Tail>>,
            Nat<N>) { return recur(recur, type<Head>, nat<N + 1>); }),
        type<T>,
        nat<0>);
   };
   ...
   /**
    * @brief Return a vector like the input vector with an element
   ...
   /**
    * @brief Return a type proxy for the last element of the nonempty
    * input vector
    */
   constexpr auto back_type = curry(nat<1>, []<NonemptyVector T>(T xs) {
      return type_of(back(xs));
   });

   constexpr auto vector_head_type = curry(nat<1>, []<Vector T>(T xs) {
      return type_of(pop_back(xs));
   });

   template<class T>
   concept IntegerComparison = requires(T cmp, integer x, integer y) {
      { cmp(x, y) } -> same_as<bool>;
   };

   template<class T, auto cmp, integer N>
   concept VectorWithLength = Vector<T> &&
     IntegerComparison<decltype(cmp)> &&
   ...
   constexpr auto vector_ref = curry(
     ...
     []<integer I, VectorWithLength<greater {}, I> T>(Nat<I>, T xs) {
        return vector_rref(
          nat<vector_type_length(type<T>) - I - 1>,
          xs);
     });

   template<NonemptyVector T>
   using BackType =
     typename decltype(flatmap(back_type, type<T>))::type;
