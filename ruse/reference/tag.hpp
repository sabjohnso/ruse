...
namespace ruse::reference {
   ...
   /**
    * @brief A concept for `HasNameType` types where `name_type` is an
    * empty type.
    */
   template<typename T>
   concept HasEmptyNameType =
     HasNameType<T> && Empty<typename T::name_type>;

   /**
    * @brief A concept for types, which value wappers that have an empty
    * name type.
    */
   template<typename T>
