...
namespace ruse::reference {
   ...
   struct stateful {
      ...
      constexpr stateful(F input) : run_state{input} {}
   };

   /**
    * @brief Return `true` if the input is a type proxy for a stateful
   ...
   /**
    * @brief The concept of a stateful computation that can run  over
   ...
   /**
    * @brief Return true for stateful input. Otherwise, return false
    * otherwise.
    */
   constexpr auto is_stateful = []<typename T>(auto) {
      return Stateful<T>;
   };

   /**
    * @brief Return the result and state from a stateful computation
    * with the input state as the initial state
    */
   ...
   /**
    * @brief Return the state as the return value in stateful
    * computation
    */
   constexpr auto get_state =
     stateful{[](auto s) { return list(s, s); }};

   /**
    * @brief Return a value selected from the state in a stateful
    * computation
    */
   constexpr auto select_state = [](auto f) {
      return fmap(f, get_state);
   };
   ...
   /**
    * @begin Run a stateful computation, returning the final value and
   ...
   /**
    * @end Run a stateful computation, returning the final value and
    * discarding the final state.
    */
