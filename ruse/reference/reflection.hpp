...
namespace ruse::reference {
   ...
   constexpr const auto& evaluate_enum_values =
     ruse::details::static_const<
   ...
   string_view_count(string_view pattern, string_view str) {
      const auto recur =
        [ = ](auto recur, string_view str, integer accum) -> integer {
         ...
         return pos == string_view::npos ? accum
                                         : recur(
                                             recur,
                                             str.substr(
                                               pos + std::size(pattern),
                                               string_view::npos),
                                             accum + 1);
      };
   ...
   strip_qualification(string_view str) {
      return str.substr(
        str.find_last_of(string_view("::")) + 1,
        string_view::npos);
   }
   ...
   template<typename T>
   constexpr auto
     enum_names = []<auto... Index>(index_sequence<Index...>) {
      return list(get_enum_name(
   ...
   template<Enum T>
   constexpr auto
     enum_names_short = []<auto... Index>(index_sequence<Index...>) {
      return list(get_short_enum_name(
        hoisted<list_ref(nat<Index>, enum_values<T>)>{})...);
   }
   (make_index_sequence<length(enum_values<T>)>());
   ...
   constexpr auto lookup_enum_name = []<Enum T>(T x) {
      const auto recur =
        [ = ](auto recur, integer index) -> string_view {
   ...
   constexpr auto count_aggregate_members = []<typename T>(Type<T>) {
      constexpr auto recur =
        []<typename... Args>(auto recur, Args... args) {
   ...
   constexpr auto
     aggregate_member_count = count_aggregate_members(type<T>);
   ...
   template<Aggregate T>
   constexpr auto
     aggregate_member_types = get_aggregate_member_types(type<T>);
   ...
   struct named {
      ...
      constexpr named(value_type value) : value(value) {}
